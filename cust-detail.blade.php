...
<body>
                                    ...
                                    <button class="action-btn" onclick="toggleDropdown(this)">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            ...
                                            <path
                                                ...
                                                fill="white" />
                                            <path
                                                d="M10.5 12C10.5 11.1716 11.1716 10.5 12 10.5C12.8284 10.5 13.5 11.1716 13.5 12C13.5 12.8284 12.8284 13.5 12 13.5C11.1716 13.5 16.5 12.8284 16.5 12Z"
                                                fill="white" />
                                            <path
                                                d="M16.5 12C16.5 11.1716 17.1716 10.5 18 10.5C18.8284 10.5 19.5 11.1716 19.5 12C19.5 12.8284 18.8284 13.5 18 13.5C17.1716 13.5 16.5 12.8284 16.5 12Z"
                                                fill="white" />
    ...
    <script>
        ...
        // ===== MODAL FUNCTIONS - COMPLETE & BUG-FREE =====
        ...
        function openTransactionDetailModal(transactionId, element) {
            ...
            try {
                ...
                const dummyTransactionData = {
                    ...
                    products: [
                        { name: "Tennesse Honey", qty: 2 },
                        { name: "Old No. 7", qty: 1 }
                    ],
                    summaryItems: [
                        { label: "Tennesse Honey", value: 1125000, qty: 2, type: "product" },
                        { label: "Old No. 7", value: 2250000, qty: 1, type: "product" },
                        { label: "Old No. 7 Promo 5%", value: -337500, type: "discount" }
                    ],
                    subTotal: 4162500,
                    tax: 208125,
                    totalAmount: 4370625,
                ...
                if (productList) {
                    ...
                    dummyTransactionData.products.forEach(product => {
                ...
                // Update summary list
                const summaryList = document.getElementById('detailSummaryList');
                if (summaryList) {
                    summaryList.innerHTML = '';
                    dummyTransactionData.summaryItems.forEach(item => {
                        const summaryItem = document.createElement('div');
                        summaryItem.classList.add('summary-item');
                        if (item.type === 'discount') {
                            summaryItem.classList.add('promo-item');
                        }
                        let valueText = `Rp ${Math.abs(item.value).toLocaleString('id-ID')}`;
                        if (item.type === 'discount') {
                            valueText = `- ${valueText}`;
                            summaryItem.classList.add('discount');
                        } else if (item.qty) {
                            valueText += ` <span class="qty-multiplier">x${item.qty}</span>`;
                        }

                        summaryItem.innerHTML = `
                            <span class="summary-label">${item.label}</span>
                            <span class="summary-value">${valueText}</span>
                        `;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            historyDateInputs.forEach(input => {
                input.addEventListener('focus', function() {
                    ...
                    this.parentElement.querySelector('.calendar-icon').style.color = 'var(--Primary-200)';
                });
                ...
                input.addEventListener('blur', function() {
                    ...
                    this.parentElement.querySelector('.calendar-icon').style.color = 'var(--Gray-100)';
                });
            });
